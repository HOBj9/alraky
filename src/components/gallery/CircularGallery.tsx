...
class App {
  ...
  constructor(
    ...
    this.scheduleCheck = () => {
      ...
      self.checkTimer = setTimeout(() => {
        self.checkTimer = null;
        if (self.destroyed || !self.medias?.[0]) return;
        const width = self.medias[0].width;
        const itemIndex = Math.round(Math.abs(self.scroll.target) / width);
        const item = width * itemIndex;
        self.scroll.target = self.scroll.target < 0 ? -item : item;
      }, 200);
    };
  }
  ...
  snapScroll = () => {
    if (this?.destroyed || !this?.medias?.[0]) return;
  ...
  createMedias(
    ...
    const defaultItems: CircularGalleryItem[] = [
      { image: "/images/products/tomato-paste.jpg", text: "Tomato Paste" },
      { image: "/images/products/olive-oil.jpg", text: "Olive Oil" },
    ];
